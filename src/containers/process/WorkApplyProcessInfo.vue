    ...
    <div class="work-report-daily">
      ...
      <div>
        <Row>
          <i-Col>
            <div class="title">
              <h3><span>{{ userName }}</span>的加班申请</h3>
            </div>
          </i-Col>
        </Row>
                  ...
                  <Row>
                    <i-Col :lg="{span:12}" :md="{span:16}" :sm="{span:20}" :xs="{span:24}">
                      <FormItem label="开始时间" prop="startTime">
                        <span>{{ item.overTimeBeginTime }}</span>
                      </FormItem>
                    </i-Col>
                  </Row>
                </div>
              ...
              <i-Col span="12">
                <div class="card">
                  <Row>
                    <i-Col :lg="{span:12}" :md="{span:16}" :sm="{span:20}" :xs="{span:24}">
                      <FormItem label="结束时间">
                        <span>{{ item.overTimeEndTime }}</span>
                      </FormItem>
                    </i-Col>
                  </Row>
                </div>
              ...
              <i-Col>
                <div class="card">
                  <Row>
                    <i-Col :lg="{span:12}" :md="{span:16}" :sm="{span:20}" :xs="{span:24}">
                      <FormItem label="加班时长">
                        <span>{{item.overTimeHour}}小时</span>
                      </FormItem>
                    </i-Col>
                  </Row>
                </div>
  ...
  export default {
    ...
    data () {
      return {
        ...
        selectBillType: this.$route.query.selectBillType,               // 查询方式（加班/加班/休假/调休/全部）
      ...
      getBillInfo () {
        this.$ajax.get(`/myApply/queryApproveDetailByBillCode`, {
          ...
          if (response.data.code === '000000') {
            ...
            this.nCHREvectionApplyDeatil = response.data.data.nchroverTimeApplyDetail
...
//    点击展开明细
      handleIsOpen () {
        if (this.isOpen === true) {
          this.isOpen = false
          this.isOpenText = '展开加班明细'
        } else {
          this.isOpen = true
          this.isOpenText = '收起加班明细'
        }
      },
//    点击评阅情况
      handleOpinion () {
          ...
          if (response.data.code === '000000' && response.data.data.dataList.length !== 0) {
            ...
            if (nextData.billType === this.$route.query.billType) {
              ...
              var data = {
                ...
                selectBillType: this.selectBillType,               // 查询方式（加班/加班/休假/调休/全部）
                index: this.index - 1
              }
