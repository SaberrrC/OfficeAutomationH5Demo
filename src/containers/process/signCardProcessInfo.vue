    ...
    <div class="work-report-daily">
      ...
      <div>
        <Row>
          <i-Col>
            <div class="title">
              <h3><span>{{ userName }}</span>的签卡申请</h3>
            </div>
          </i-Col>
        </Row>
  ...
  export default {
    ...
    data () {
      return {
        ...
        selectBillType: this.$route.query.selectBillType,               // 查询方式（加班/签卡/休假/调休/全部）
      ...
      getBillInfo () {
        this.$ajax.get(`/myApply/queryApproveDetailByBillCode`, {
          ...
          if (response.data.code === '000000') {
            ...
            this.nCHREvectionApplyDeatil = response.data.data.nchrSignDetails
            this.applyWorkFlows = response.data.data.applyWorkFlows
...
//    点击展开明细
      handleIsOpen () {
        if (this.isOpen === true) {
          this.isOpen = false
          this.isOpenText = '展开签卡明细'
        } else {
          this.isOpen = true
          this.isOpenText = '收起签卡明细'
        }
      },
//    点击评阅情况
      handleOpinion () {
            ...
            if (response.data.code === '000000' && response.data.data.dataList.length !== 0) {
              ...
              if (nextData.billType === this.$route.query.billType) {
                ...
                var data = {
                  ...
                  selectBillType: this.selectBillType,               // 查询方式（加班/签卡/休假/调休/全部）
                  index: this.index - 1
                }
