    ...
    <div class="work-report-daily">
      ...
      <div>
        <Row>
          <i-Col>
            <div class="title">
              <h3><span>{{ userName }}</span>的休假申请</h3>
            </div>
          </i-Col>
        </Row>
      ...
      getBillInfo () {
        this.$ajax.get(`/myApply/queryApproveDetailByBillCode`, {
          ...
          if (response.data.code === '000000') {
            ...
            this.billTitle.type = response.data.data.typeName
            this.billTitle.date = response.data.data.date
            this.nCHREvectionApplyDeatil = response.data.data.nchrfurloughApplyDetail
...
//    点击展开明细
      handleIsOpen () {
        if (this.isOpen === true) {
          this.isOpen = false
          this.isOpenText = '展开休假明细'
        } else {
          this.isOpen = true
          this.isOpenText = '收起休假明细'
        }
      },
//    点击评阅情况
      handleOpinion () {
