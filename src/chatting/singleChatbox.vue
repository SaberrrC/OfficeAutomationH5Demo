      ...
      <div v-for="(obj,index) in MessageHistory">
        <div>
          ...
          <div class="listContent" v-if="obj.nameType==2">
            <div v-if="obj.formType==1">
              <div class="img" ondragstart="return false;">
                <template v-if="chatType === 'singleChat'">
                  <img :src="addHttp(otherInfo.img)" :title="otherInfo.username"></template>
                <template v-if="chatType === 'chatGroup'">
                  <img :src="obj.img" :title="obj.username">
                </template>
                <template v-if="chatType === 'chatRoom'">
                  <img :src="obj.img" :title="obj.username">
                </template>
            ...
            <div v-if="obj.formType==2" ondragstart="return false;">
              <div class="img">
                <template v-if="!isGroupChat"><img :src="addHttp(otherInfo.img)" :title="otherInfo.username "></template>
                <template v-else>
                  <img :src="obj.img" :title="obj.username">
                </template>
              </div>
          ...
          <div class="listContentRight" v-else>
            <div v-if="obj.formType==1">
              <div class="img" ondragstart="return false;">
                <img :src="reImgUrl(userinfo.img)" :title="userinfo.name">
              </div>
              <div class="text" v-html="obj.value"></div>
            </div>
            <div v-if="obj.formType==2" ondragstart="return false;">
              <div class="img"><img :src="reImgUrl(userinfo.img)" :title="userinfo.name"></div>
              <div class="text" v-on:click="openFile(obj)">
                <div class="el-icon-document TXTleft"></div>
                <div class="TXTright">
                  <div class="TXTright_name">{{obj.name}}</div>
            ...
            <div v-if="obj.formType==3">
              <div class="img" ondragstart="return false;"><img :src="reImgUrl(userinfo.img)" :title="userinfo.name"></div>
              <div class="text" v-on:click="openImage(obj)">
      ...
      if (this.otherInfo.roomId && this.otherInfo.roomId !== '') {
        ...
        for (let i = 0; i < om.length; i++) {
          ...
          config.OA_URL + 'user/getinfo',
          {
            ...
            if (response.data.data[0].img == "" || response.data.data[0].img == "http://") {
              ...
              img = 'http://' + response.data.data[0].img
            }
            tmp.push({
              code:response.data.data[0].code,
              name: response.data.data[0].username,
              img:img,
              departmentName:response.data.data[0].organ,
              email:response.data.data[0].email,
              phone:response.data.data[0].phone,
              postname:response.data.data[0].postname,
              sex:response.data.data[0].sex,
              affiliation: that.otherInfo.members[i].affiliation
            })
          })
        }
        return tmp
